...
func Zap() (logger *zap.Logger) {
	//判断是否有文件夹
	if ok, _ := utils.IO.PathExists(global.GVA_CONFIG.Zap.Director); !ok {
...
func getEncoderCore(fileName string, level zapcore.LevelEnabler) (core zapcore.Core) {
	writer := utils.RotateLogs.GetWriteSyncer(fileName) // 使用file-rotatelogs进行日志分割
	return zapcore.NewCore(getEncoder(), writer, level)
}

// CustomTimeEncoder 自定义日志输出时间格式
func CustomTimeEncoder(t time.Time, enc zapcore.PrimitiveArrayEncoder) {
	enc.AppendString(t.Format(global.GVA_CONFIG.Zap.Prefix + global.GVA_CONFIG.Zap.TimeFormat))
}
